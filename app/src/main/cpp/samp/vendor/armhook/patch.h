...
public:
    ...
    static inline bool glossInitialized = false;
public:
    ...
    static uintptr_t GetAddrBaseXDL(uintptr_t addr)
    {
        ...
        if (GlossAddr(addr, &sym_addr, &sym_size, sym_name)) {
            return sym_addr;
        }
        return 0;
    }
...
//        #endif
    ...
    static uintptr_t getSym(const char* sym)
    {
        ...
        if(res == 0){
            GHandle handle = GlossOpen("libGame.so");
            if (handle) {
                size_t sym_size = 0;
                res = GlossSymbol(handle, sym, &sym_size);
    ...
    static void InstallPLT(Addr addr, Func hook_func, Orig* orig_func)
    {
        ...
        *orig_func = reinterpret_cast<Orig>(*(uintptr_t*)addr);
        GHook hook = GlossGotHook((void*)addr, (void*)hook_func, (void**)orig_func);
        if (!hook) {
    ...
    {
        UnFuck(addr);
        GHook hook = GlossGotHook((void*)addr, (void*)hook_func, nullptr);
        if (!hook) {
            *(uintptr_t*)addr = reinterpret_cast<uintptr_t>(hook_func);
        }
    }
    ...
    static void InlineHook(Ptr ptr, Func func, Orig orig)
    {
        i_set mode = I_ARM;
#ifdef __arm__
        mode = THUMBMODE((uintptr_t)ptr) ? I_THUMB : I_ARM;
#endif
#ifdef __aarch64__
        mode = I_ARM64;
#endif
        GHook hook = GlossHookAddr((void*)ptr, (void*)func, (void**)orig, false, mode);
    }

    template <typename Func, typename Orig>
    static void InlineHook(const char* sym, Func func, Orig orig)
    {
        GHook hook = GlossHookByName("libGame.so", sym, (void*)func, (void**)orig, nullptr);
    }
