...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	switch (byteRejectReason)
	{
		case REJECT_REASON_BAD_VERSION:
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_ver);
			break;

		case REJECT_REASON_BAD_NICKNAME:
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_nickname1);
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_nickname2);
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_nickname3);
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_nickname4);
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_nickname5);
			pNetGame->SetGameState(GAMESTATE_WAIT_CONNECT);
			break;

		case REJECT_REASON_BAD_MOD:
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_mod);
			break;
		case REJECT_REASON_BAD_PLAYERID:
			if (pUI) pUI->chat()->addInfoMessage(localisation_bad_id);
			break;
	}
...
void WorldVehicleAdd(RPCParameters* rpcParams)
{
	...
	if(CHook::CallFunction<int>(g_libGTASA + (VER_x32 ? 0x3863EC + 1 : 0x45D0C8), VehicleInfo.iVehicleType) == -1) {
		if (pUI)
			pUI->chat()->addDebugMessage("Error: try create vehicle with not valid vehicle model %d", VehicleInfo.iVehicleType);
		return;
	}

	if (VehicleInfo.iVehicleType == 537 || VehicleInfo.iVehicleType == 538) {
		Log("Warning: vehicle model is a train, idi naxui");
		return;
	}
