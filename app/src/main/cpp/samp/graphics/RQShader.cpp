...
void BuildPixelSource(uint32 flags) {
    ...
    if (flags & FLAG_LIGHTING)
        buff << "varying lowp float Out_AO;";
    ...
    if (flags & FLAG_LIGHTING)
        buff << "fcolor.xyz *= Out_AO;";

    if (flags & FLAG_GAMMA)
        buff << "fcolor.xyz += fcolor.xyz * 0.4;"; // [MOD] было 0.5, чуть мягче

    if (flags & FLAG_LIGHTING)
        buff << "fcolor.xyz = pow(clamp(fcolor.xyz, 0.0, 1.0), vec3(0.92));";
...
void BuildVertexSource(uint32 flags) {
    ...
    if (flags & FLAG_LIGHTING) {
        ...
        if (flags & (FLAG_LIGHT1 | FLAG_LIGHT2 | FLAG_LIGHT3)) {
            if (flags & FLAG_LIGHT1) {
                if (GetMobileEffectSetting() == 3 && (flags & (FLAG_BACKLIGHT | FLAG_BONE3 | FLAG_BONE4))) {
                    ...
                    buff << "float NdotL1 = dot(DirLightDirection, WorldNormal) * 0.5 + 0.5;"
                         << "NdotL1 = NdotL1 * NdotL1;"
                         << "diffColor += NdotL1 * DirLightDiffuseColor;";
                }
            }
            if (flags & FLAG_LIGHT2) {
                buff << "float NdotL2 = dot(DirLight2Direction, WorldNormal) * 0.5 + 0.5;"
                     << "NdotL2 = NdotL2 * NdotL2;"
                     << "diffColor += NdotL2 * DirLight2DiffuseColor;";
            }
            if (flags & FLAG_LIGHT3) {
                buff << "float NdotL3 = dot(DirLight3Direction, WorldNormal) * 0.5 + 0.5;"
                     << "NdotL3 = NdotL3 * NdotL3;"
                     << "diffColor += NdotL3 * DirLight3DiffuseColor;";
            }
        ...
        buff << "float aoFactor = clamp(WorldNormal.z * 0.5 + 0.7, 0.35, 1.0);"
             << "aoFactor = aoFactor * aoFactor;"   // квадратичный фоллоф
             << "Out_AO = aoFactor;";
