...
void Plugin::MainLoop()
{
    ...
    {
        PluginConfig::SetSoundEnable(false);
        PluginConfig::SetMicroEnable(false);
        ...
        while (Network::ReceiveControlPacket()) {}
        while (Network::ReceiveVoicePacket()) {}
        return;
    }
    if (pSettings && pSettings->Get().bVoiceChatEnable)
    {
        ...
        {
            PluginConfig::SetSoundEnable(true);
            Playback::SetSoundEnable(true);
        }
        if (!PluginConfig::GetMicroEnable())
        {
            PluginConfig::SetMicroEnable(true);
