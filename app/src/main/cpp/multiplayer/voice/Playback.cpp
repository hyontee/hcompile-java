...
void Playback::Tick() noexcept
{
    ...
    const int targetGlobalVolume = PluginConfig::GetSoundEnable()
        ? 100 * PluginConfig::GetSoundVolume()
        : 0;
    static int lastGlobalVolume = -1;
    if(targetGlobalVolume != lastGlobalVolume)
    {
        BASS_SetConfig(BASS_CONFIG_GVOL_STREAM, targetGlobalVolume);
        lastGlobalVolume = targetGlobalVolume;
    }
}
