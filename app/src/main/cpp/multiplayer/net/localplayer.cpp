...
// Показывает hud_seat_button только когда рядом есть машина и игрок не в ней
void CLocalPlayer::ProcessPassengerButtonVisibility() {
    static bool bLastState = false;
    bool bShouldShow = false;

    if (pNetGame && m_pPlayerPed && !m_pPlayerPed->IsInVehicle()) {
        CVehiclePool* pVehiclePool = pNetGame->GetVehiclePool();
        if (pVehiclePool) {
            VEHICLEID ClosetVehicleID = pVehiclePool->FindNearestToLocalPlayerPed();
            CVehicle* pVehicle = pVehiclePool->GetAt(ClosetVehicleID);
            if (pVehicle && pVehicle->m_pVehicle->GetDistanceFromLocalPlayerPed() < 8.0f) {
                bShouldShow = true;
            }
        }
    }

    if (bShouldShow != bLastState) {
        bLastState = bShouldShow;
        CHud::toggleSeatButton(bShouldShow);
    }
}
...
bool CLocalPlayer::Process()
{
    ...
    CLocalPlayer::ProcessPassengerButtonVisibility();
