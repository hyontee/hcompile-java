...
void ScrSetVehicleNumberPlate(RPCParameters* rpcParams)
{
	...
	// pVehicle->SetPlateText(szPlateName) писал в поле, которое реально
	// нигде не читается движком при рендере — номер сохранялся, но не рисовался.
	// В x32 номер выставляется через нативную SCM-команду движка (as in
	// vanilla GTA:SA mission scripting: 0674: set_car $vehicle numberplate 'text'),
	// которая корректно подключена к реальному рендеру таблички.
	pVehicle->SetPlateText(szPlateName);
	ScriptCommand(&set_car_numberplate, pVehicle->m_dwGTAId, szPlateName);
}
