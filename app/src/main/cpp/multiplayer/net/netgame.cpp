...
#define RPC_AZS  0x35
#define RPC_NUM_SHOW  0x41
#define RPC_NUM_BUY_RESULT  0x44
...
void CNetGame::Packet_CustomRPC(Packet *p) {
    ...
    switch (rpcID) {
        ...
        {
            uint32_t home = 0, fraction = 0;
            bs.Read(home);
            bs.Read(fraction);

            FLog("RPC_SPAWN_SELECT home=%u fraction=%u", home, fraction);
            pJavaWrapper->ShowSpawn(home, fraction);
            break;
        }
        case RPC_GPS_VIEW:
        {
            ...
            FLog("RPC_GPS_VIEW value=%u", value);
            pJavaWrapper->ShowGPS(value);
        ...
        case RPC_GREENZONE_VIEW:
        {
            ...
            FLog("RPC_GREENZONE_VIEW value=%u", value);
            pJavaWrapper->ShowGreenZone(value);

            break;
        }
        case RPC_AZS:
        {
            uint32_t fuelId = 0, maxFuel = 0, price = 0, balance = 0;
            float currentFuel = 0;
            ...
            FLog("RPC_AZS fuelId=%u maxFuel=%u currentFuel=%f price=%u balance=%u", fuelId, maxFuel, currentFuel, price, balance);

            if (fuelId <= 5)
                pJavaWrapper->ShowAZS(fuelId, maxFuel, currentFuel, price, balance);
        ...
        case RPC_NUM_SHOW:
        {
            ...
            FLog("RPC_NUM_SHOW balance=%u", balance);
            pJavaWrapper->ShowNumbers(balance);

            break;
        }
        case RPC_NUM_BUY_RESULT:
        {
            uint32_t status = 0, reason = 0;
            bs.Read(status);
            bs.Read(reason);

            FLog("RPC_NUM_BUY_RESULT status=%u reason=%u", status, reason);
            pJavaWrapper->BuyNumberResult(status, reason);
