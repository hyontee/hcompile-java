...
Java_com_brilliant_game_ui_hud_Hud_HudInit(JNIEnv *env, jobject thiz) {
...
void CHud::toggleSeatButton(bool toggle) {
    JNIEnv* env = CJavaWrapper::GetEnv();
    if(!env) return;

    jclass clazz = env->GetObjectClass(thiz);
    jmethodID method = env->GetMethodID(clazz, "toggleSeatButton", "(Z)V");
    if(method) env->CallVoidMethod(thiz, method, toggle);
}
/*
...
extern "C"
JNIEXPORT void JNICALL Java_com_brilliant_game_ui_hud_Hud_alt(JNIEnv *env, jobject thiz) {
LocalPlayerKeys.bKeys[ePadKeys::KEY_WALK] = true;
}

extern "C"
JNIEXPORT void JNICALL Java_com_brilliant_game_ui_hud_Hud_N(JNIEnv *env, jobject thiz) {
LocalPlayerKeys.bKeys[ePadKeys::KEY_NO] = true;
}

extern "C"
JNIEXPORT void JNICALL Java_com_brilliant_game_ui_hud_Hud_SetRadarBgPos(JNIEnv *env, jobject thiz,
...
extern "C"
JNIEXPORT void JNICALL Java_com_brilliant_game_ui_hud_Hud_SetRadarPos(JNIEnv *env, jobject thiz,
...
extern "C"
JNIEXPORT void JNICALL Java_com_brilliant_game_ui_hud_Hud_nativeBoolShowHud(JNIEnv *env, jobject thiz, jboolean toggle) {
CHud::bIsShow = toggle;
}

extern "C"
JNIEXPORT void JNICALL Java_com_brilliant_game_ui_hud_Hud_sendG(JNIEnv *env, jobject thiz) {
if (pNetGame) {
    CPlayerPool* pPlayerPool = pNetGame->GetPlayerPool();
    if (pPlayerPool) {
        CLocalPlayer* pLocalPlayer = pPlayerPool->GetLocalPlayer();
        if (pLocalPlayer) {
            // bNeedEnterVehicle сейчас никем не устанавливается в true — "вооружаем" его здесь,
            // GbuttonEnterVehicleAsPassenger() сама сбросит флаг обратно в false после проверки.
            bNeedEnterVehicle = true;
            pLocalPlayer->GbuttonEnterVehicleAsPassenger();
        }
    }
}
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_brilliant_game_ui_hud_Hud_nativeClick(JNIEnv *env, jobject thiz, jint buttonId) {
switch (buttonId) {
...
break;
    case CHud::BUTTON_ALT: {
        LocalPlayerKeys.bKeys[ePadKeys::KEY_WALK] = true;
        std::thread([](){
            std::this_thread::sleep_for(std::chrono::milliseconds(50));
            LocalPlayerKeys.bKeys[ePadKeys::KEY_WALK] = false;
        }).detach();
        break;
    }
    case CHud::BUTTON_H: {
