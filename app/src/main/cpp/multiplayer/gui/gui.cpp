...
bool UI::initialize()
{
    ...
    if(VER_x32)
    {
        d_label1 = new Label(cryptor::create("Brilliant mobile x32").decrypt(), ImColor(1.0f, 1.0f, 1.0f), true, UISettings::fontSize() / 2);
        this->addChild(d_label1);
        d_label1->setPosition(ImVec2(3.0, 3.0));
    }
    else
    {
        d_label1 = new Label(cryptor::create("PRIVATE (arm64-v8a)").decrypt(), ImColor(1.0f, 1.0f, 1.0f), true, UISettings::fontSize() / 2);
        this->addChild(d_label1);
        d_label1->setPosition(ImVec2(3.0, 3.0));
    }
