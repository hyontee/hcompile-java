package com.brilliant.game.core;

import android.os.Bundle;
import android.util.Log;

import com.brilliant.game.ui.attach.AttachEdit;
import com.brilliant.game.ui.dialog.DialogManager;
import com.brilliant.game.ui.hud.Hud;
import com.brilliant.game.ui.hud.Speedometr;
import com.brilliant.game.ui.hud.Keyboard;
import com.brilliant.game.ui.hud.Notification;
import com.brilliant.game.ui.loading.LoadingScreen;
import com.brilliant.game.ui.tab.PlayerData;
import com.brilliant.game.ui.tab.Tab;
import com.brilliant.game.ui.reg.Reg;
import com.brilliant.game.ui.auth.SpawnSelector;
import com.brilliant.game.ui.azs.AZS;
import com.brilliant.game.ui.numbers.Numbers;
...
public class Samp extends GTASA implements Keyboard.InputListener, HeightProvider.HeightListener {
    ...
    public native void sendBuyNumber(int country, int isCustom, String number, String region);
    ...
    public void ShowSpawn(int home, int fraction) { runOnUiThread(() -> { mSpawnSelector.show(home != 0, fraction != 0); });}
    public void HideSpawn() { runOnUiThread(() -> { mSpawnSelector.hide();});}
    public void ShowAZS(int fuelId, int maxFuel, float currentFuel, int price, int balance) { runOnUiThread(() -> { mAZS.show(fuelId, maxFuel, currentFuel, price, balance); });}

    public void ShowGPS(int value) {
        runOnUiThread(() -> {
            android.widget.ImageView v = findViewById(R.id.rpc_gps);
            if (v != null) v.setVisibility(value == 1 ? android.view.View.VISIBLE : android.view.View.GONE);
        });
    }

    public void ShowGreenZone(int value) {
        runOnUiThread(() -> {
            android.widget.ImageView v = findViewById(R.id.rpc_greenzone);
            if (v != null) v.setVisibility(value == 1 ? android.view.View.VISIBLE : android.view.View.GONE);
        });
    }
    public void ShowNumbers(int balance) { runOnUiThread(() -> { mNumbers.show(balance); });}
    public void BuyNumberResult(int status, int reason) { runOnUiThread(() -> { mNumbers.buyResult(status != 0, reason); });}
    public void HideAZS() { runOnUiThread(() -> { mAZS.hide();});}
    ...
    public Speedometr getSpeedometr() {
        return this.mSpeedometr;
    }
